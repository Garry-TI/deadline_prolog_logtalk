...
:- object(utils).
    ...
    describe_room_contents(Room) :-
        % Describe NPCs
        forall(
            ( state::location(NPC, Room), state::has_flag(NPC, person),
              \+ state::has_flag(NPC, invisible) ),
            ( catch(NPC::desc(D), _, D = NPC),
              format("~w is here.~n", [D]) )
        ),
        % Describe objects (non-NPC, visible, not ndescbit)
        forall(
            ( state::location(Obj, Room),
    ...
    describe_object_in_room(Obj) :-
        ( state::has_flag(Obj, touchbit) ->
            % Object has been seen: use LDESC
            ( catch(Obj::ldesc(D), _, fail) -> writeln(D)
            ; catch(Obj::desc(D), _, D = Obj),
              format("There is ~w here.~n", [D])
            )
        ;
            % First time: use FDESC if available
            ( catch(Obj::fdesc(D), _, fail) -> writeln(D)
            ; catch(Obj::ldesc(D), _, fail) -> writeln(D)
            ; catch(Obj::desc(D), _, D = Obj),
              format("There is ~w here.~n", [D])
            )
